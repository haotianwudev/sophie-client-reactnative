...
import { GET_STOCK_TECHNICALS } from '../lib/graphql/queries';
...
type Props = StackScreenProps<RootStackParamList, 'TechnicalAnalysis'>;

const TechnicalAnalysisScreen = ({ route, navigation }: Props) => {
  const { ticker } = route.params;
  const colorScheme = useColorScheme();
  const isDark = colorScheme === 'dark';
  
  // Fetch technical data
  const { data, loading, error } = useQuery(GET_STOCK_TECHNICALS, {
    variables: { ticker },
    fetchPolicy: 'cache-and-network',
  });

  const technicalData = data?.latestTechnicals;
        ...
        <View style={[styles.stockHeading, isDark && styles.darkStockHeading]}>
          <View style={styles.headingContent}>
            ...
            <View>
              ...
              <Text style={[styles.sectionTitle, isDark && styles.darkMutedText]}>Technical Analysis</Text>
            </View>
          </View>
        </View>
